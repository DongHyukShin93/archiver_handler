...
def main():
    ...
    path_cp = os.path.join(
        path_aux, "CheckPoints", args.dataset, str_aux, model_name, str(args.dim_out)
    )
    path_results = os.path.join(
        path_aux, "Results", args.dataset, str_aux, model_name, str(args.dim_out)
    )
    ...
    transform_image = transforms.Compose(
        [transforms.Resize((args.im_sz, args.im_sz)), transforms.ToTensor()]
    )
    transform_sketch = transforms.Compose(
        [transforms.Resize((args.sk_sz, args.sk_sz)), transforms.ToTensor()]
    )
    ...
    if args.dataset == "Sketchy":
        ...
        photo_sd = ""
        sketch_sd = ""
        splits = utils.load_files_tuberlin_zeroshot(
            root_path=root_path,
            photo_dir=photo_dir,
            sketch_dir=sketch_dir,
            photo_sd=photo_sd,
    ...
    if args.gzs_sbir > 0:
        ...
        if args.dataset == "Sketchy" and args.filter_sketch:
            _, idx_sk = np.unique(
                [f.split("-")[0] for f in tr_fls_sk_], return_index=True
            )
    ...
    data_test_sketch = DataGeneratorSketch(
        args.dataset,
        root_path,
        sketch_dir,
        sketch_sd,
    ...
    data_test_image = DataGeneratorImage(
        args.dataset,
        root_path,
        photo_dir,
        photo_sd,
...
def validate(valid_loader_sketch, valid_loader_image, sem_pcyc_model, epoch, args):
    ...
    str_sim = (
        np.expand_dims(acc_cls_sk, axis=1) == np.expand_dims(acc_cls_im, axis=0)
    ) * 1
    ...
    valid_data = {
        ...
        "sim_euc": sim_euc,
        "time_euc": time_euc,
        "aps@all_bin": apsall_bin,
        "aps@200_bin": aps200_bin,
        "prec@100_bin": prec100_bin,
        "prec@200_bin": prec200_bin,
        "sim_bin": sim_bin,
        "time_bin": time_bin,
        "str_sim": str_sim,
    }
