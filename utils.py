...
def get_coarse_grained_samples(
    classes, fls_im, fls_sk, set_type="train", filter_sketch=True
):
...
def load_files_sketchy_zeroshot(
    root_path,
    split_eccv_2018=False,
    filter_sketch=False,
    photo_dir="photo",
    sketch_dir="sketch",
    ...
    fls_sk = np.array(
        [
            "/".join(f.split("/")[-2:])
            for f in glob.glob(os.path.join(path_sk, "*/*.png"))
        ]
    )
    fls_im = np.array(
        [
            "/".join(f.split("/")[-2:])
            for f in glob.glob(os.path.join(path_im, "*/*.jpg"))
        ]
    )
    ...
    if split_eccv_2018:
        ...
        va_classes = np.random.choice(
            np.setdiff1d(classes, tr_classes), int(0.1 * len(classes)), replace=False
        )
        te_classes = np.setdiff1d(classes, np.union1d(tr_classes, va_classes))

    idx_tr_im, idx_tr_sk = get_coarse_grained_samples(
        tr_classes, fls_im, fls_sk, set_type="train", filter_sketch=filter_sketch
    )
    idx_va_im, idx_va_sk = get_coarse_grained_samples(
        va_classes, fls_im, fls_sk, set_type="valid", filter_sketch=filter_sketch
    )
    idx_te_im, idx_te_sk = get_coarse_grained_samples(
        te_classes, fls_im, fls_sk, set_type="test", filter_sketch=filter_sketch
    )
...
def load_files_tuberlin_zeroshot(
    root_path, photo_dir="images", sketch_dir="sketches", photo_sd="", sketch_sd=""
):
    ...
    fls_sk = np.array(
        [os.path.join(f.split("/")[-2], f.split("/")[-1]) for f in fls_sk]
    )
    clss_sk = np.array([f.split("/")[-2] for f in fls_sk])
    ...
    va_classes = np.random.choice(
        np.setdiff1d(classes, tr_classes), int(0.06 * len(classes)), replace=False
    )
    te_classes = np.setdiff1d(classes, np.union1d(tr_classes, va_classes))
    ...
    idx_va_im, idx_va_sk = get_coarse_grained_samples(
        va_classes, fls_im, fls_sk, set_type="valid"
    )
    idx_te_im, idx_te_sk = get_coarse_grained_samples(
        te_classes, fls_im, fls_sk, set_type="test"
    )
    ...
    for i, isk in enumerate(ind_sk):
        ...
        if save_image:
            ...
            if not os.path.isdir(sdir_op):
                os.makedirs(sdir_op)
            sk = (
                Image.open(os.path.join(dir_sk, fls_sk[isk]))
                .convert(mode="RGB")
                .resize(im_sz)
            )
        ...
        for j, iim in enumerate(ind_im):
            ...
            if save_image:
                im = (
                    Image.open(os.path.join(dir_im, fls_im[iim]))
                    .convert(mode="RGB")
                    .resize(im_sz)
                )
                im.save(
                    os.path.join(
                        sdir_op, str(j + 1) + "_" + str(str_sim[isk][iim]) + ".png"
                    )
                )
        fp.write("\n")
    fp.close()
